...
pub fn draw_maze(maze: &Maze) {
    ...
    for _ in 1..=(maze.width + 2 * (BORDER_WIDTH_X as usize - 1)) {
    ...
    for extra_space in 1..BORDER_WIDTH_Y {
        println!(
            ...
            termion::cursor::Goto(
                TERMION_OFFSET_X + 2 * BORDER_WIDTH_X + maze.width as u16 - 1,
    ...
    for (idx_row, row) in maze.data.iter().enumerate() {
        print!(
            "{}{}{}",
            termion::cursor::Goto(
                TERMION_OFFSET_X,
    ...
    for extra_space in 1..BORDER_WIDTH_Y {
        println!(
            ...
            termion::cursor::Goto(
                TERMION_OFFSET_X + 2 * BORDER_WIDTH_X + maze.width as u16 - 1,
                TERMION_OFFSET_Y + BORDER_WIDTH_Y + maze.height as u16 + extra_space - 1
            ),
            SYMBOL_MAZE_BORDER_RIGHT,
        );
    }
